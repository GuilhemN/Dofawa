...
abstract class Level3AjaxFormController extends Controller
{
    ...
    public function fetchAction($l1id, $l2id, $l3id)
    {
        ...
        $ent = $dm->getRepository(static::ENTITY_NAME)->find($l3id);
        if ($ent === null)
    ...
    public function storeAction(Request $req, $l1id, $l2id, $l3id)
    {
        ...
        $data = $ent->exportData();
        if ($l1id === 'new') {
            $location = $this->get('router')->generate(static::ROUTE_NAME, [ 'l1id' => $l1id, 'l2id' => $l2id, 'l3id' => $ent->getId() ], true);
    ...
    protected abstract function createEntity(Request $req, ObjectManager $dm, $l1id, $l2id, $l3id);
