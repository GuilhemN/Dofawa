    ...
    protected function doImport($conn, $beta, $release, $db, array $locales, $flags, OutputInterface $output = null, ProgressHelper $progress = null)
    {
        ...
        foreach ($all as $row) {
            ...
            if ($write && !$cached)
                ...
                $this->dm->createQuery('DELETE FROM DofItemsBundle:ItemComponent s  WHERE s.id IN (SELECT cp.id FROM DofItemsBundle:ItemTemplate i JOIN i.components cp WHERE i.id = ' . $item->getId() . ' )')->execute() ;
