...
class EffectTemplateImporter extends AbstractGameDataImporter
{
    ...
    protected function doImport($conn, $beta, $release, $db, array $locales, $flags, OutputInterface $output = null, ProgressHelper $progress = null)
    {
        ...
        if (!$beta && $write) {
            $this->dm->createQuery('UPDATE DofCharacterBundle:EffectTemplate s SET s.deprecated = true')->execute();
        }
        ...
        $repo = $this->dm->getRepository('DofCharacterBundle:EffectTemplate');
        $rowsProcessed = 0;
        if ($output && $progress) {
